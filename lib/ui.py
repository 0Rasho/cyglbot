...
class ChatUI:
    def __init__(self, stdscr, userlist_width=20):
        ...
        chatbuffer_hwyx = (curses.LINES - 1, curses.COLS-userlist_width-1,
                           0, userlist_width + 1)
    ...
    def redraw_userlist(self):
        ...
        for i, name in enumerate(self.userlist):
            if i >= h:
                break
            #name = name.ljust(w - 1) + "|"
            color=self.userlist_color[name]
            self.win_userlist.addstr(i, 0, name[:w - 1], curses.color_pair(color))
    ...
    def redraw_chatbuffer(self):
        ...
        for i in range(min(h, len(self.linebuffer))):
            try:
                a=self.linebuffer[j].split(':')[0].strip()
                color=self.userlist_color[a]
                prev=color
                self.win_chatbuffer.addstr(i, 0, self.linebuffer[j])
                #self.win_chatbuffer.addstr(i, 0, self.linebuffer[j], curses.color_pair(color))
            except:
                #self.win_chatbuffer.addstr(i, 0, self.linebuffer[j], curses.color_pair(prev))
                self.win_chatbuffer.addstr(i, 0, self.linebuffer[j])
            j += 1
